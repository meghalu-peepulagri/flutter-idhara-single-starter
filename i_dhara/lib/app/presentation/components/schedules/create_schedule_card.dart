...
class ScheduleFormState extends State<ScheduleForm> {
  ...
  late Set<int> selectedDays; // 0=Sun, 1=Mon ... 6=Sat
  ...
  void initState() {
    ...
    // Default start time to exact current time if today
  ...
  Set<int> get _validDays {
    ...
    for (int i = 0; i < rangeLength; i++) {
      // Dart weekday: 1=Mon..7=Sun → convert to Sun=0, Mon=1..Sat=6
  ...
  void _openTimePicker(bool isStart) {
    ...
    if (isStart && _isStartDateToday) {
      // On today's date, don't allow picking a past time
      final now = DateTime.now();
  ...
  Future<void> _openCalendarDialog() async {
    ...
    if (results != null && results.isNotEmpty && mounted) {
      setState(() {
        ...
        // Remove any selected days that are no longer within the new date range
        selectedDays.retainWhere((d) => _validDays.contains(d));

        // Reset start time based on whether the new start date is today
        ...
        // Reset end time when start date changes
        endHour = 0;
        endMinute = 0;
      });
    }
