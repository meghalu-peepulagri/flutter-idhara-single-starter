...
class _MotorDetailsTabBarState extends State<MotorDetailsTabBar>
  ...
  static const _tabs = [
    ('Mode', 0, 'assets/images/Mode.svg'),
    ('Schedule', 1, 'assets/images/schedule.svg'),
    ('Analytics', 2, 'assets/images/Graph.svg'),
    ('Logs', 3, 'assets/images/Logs.svg'),
  ];

  // Below this the 4 tabs no longer fit on one line without scrolling
  // (verified against the narrowest supported phone width) — above it,
  // e.g. on iPad, there's room to spread the tabs evenly across the
  // full width instead of leaving them left-aligned with empty space.
  static const _fillWidthBreakpoint = 500.0;
      ...
      child: Padding(
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            if (constraints.maxWidth >= _fillWidthBreakpoint) {
              return Row(
                children: [
                  for (final tab in _tabs)
                    Expanded(
                      child: _buildSvgTab(context, tab.$1, tab.$2, tab.$3),
                    ),
                ],
              );
            }
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: Row(
                      children: [
                        for (final tab in _tabs)
                          _buildSvgTab(context, tab.$1, tab.$2, tab.$3),
                      ],
                    ),
                  ),
                ),
