...
class AnalyticsController extends GetxController {
  ...
  var canChangeMode =
      true.obs; // NEW: Reactive state for mode control eligibility
  ...
  Future<void> _initializeSequentially() async {
    ...
    _updateCanChangeMode(); // NEW: Initial update after sequential init
  }
  ...
  Future<void> _initializeMqtt() async {
    ...
    // Initial update
    _updateFromMqttData();
    _updateCanChangeMode(); // NEW: Update after MQTT stabilization
  ...
  void _updateFromMqttData() {
    ...
    _updateCanChangeMode(); // NEW: Recompute control eligibility on data update
  }
  ...
  // NEW: Signal bars computation (adapted from MotorCardWidget)
  ...
  // NEW: Update mode change eligibility (network + available + signal > 0, matching card's mode logic)
  ...
  Future<void> handleModeChange(int newModeIndex) async {
    if (!mqttInitialized || isWaitingForModeAck.value || !canChangeMode.value) {
      // NEW: Add canChangeMode check
      if (kDebugMode)
        print(
            '⚠ Analytics: Cannot change mode - MQTT not ready, waiting for ACK, or ineligible (network/signal)');
      return;
    }
  ...
  Future<void> fetchMotorDetails() async {
    ...
    try {
      ...
      _updateCanChangeMode(); // NEW: Recompute after motor details update
    }
  }
